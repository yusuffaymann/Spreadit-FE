            ...
            {loading === false && <div className={styles.body}>
              {postArray.map((postObject, index) => (
                    <div className={styles.post} key={index}>
                      <MiniaturePost postId={postObject._id} subRedditName={postObject.community} subRedditPicture={subArray[index].image} subRedditDescription={subArray[index].description} subRedditBanner={subArray[index].communityBanner} postTitle={postObject.title} attachments={postObject.attachments} upVotes={postObject.votesUpCount - postObject.votesDownCount} comments={postObject.commentsCount} isNSFW={postObject.isNsfw} isSpoiler={postObject.isSpoiler} isMember={subscribedArray[index].isSubscribed}/>
                    </div>))}
            </div>}
        </div>
    );
